...
struct WorkstreamStoreTests {
    ...
    func exactSourceEventReplayDeduplicates() {
        ...
        let firstIngestion = store.ingest(first)
        #expect(firstIngestion.inserted)
        let itemId = firstIngestion.item.id
        store.markResolved(itemId, decision: .permission(.once))
        let replayIngestion = store.ingest(WorkstreamEvent(
            sessionId: "copilot-session",
            hookEventName: .permissionRequest,
            source: "copilot",
            toolName: "shell",
        ...
        #expect(store.items.count == 1)
        #expect(!replayIngestion.inserted)
        #expect(replayIngestion.item.id == itemId)
    ...
    @Test("Same source event id remains distinct across sessions")
    func sourceEventIdentityIncludesSession() {
        let store = WorkstreamStore(ringCapacity: 10)
    ...
    @Test("Persisted duplicate updates collapse to the latest row")
    func persistedDuplicateUpdatesCollapse() async throws {
